Shader "VolumeIntersection/TopDownProjection"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 objectSpacePos = mul(unity_WorldToObject, float4(worldPos, 1.0)).xyz;

                float2 objectSpaceUV = TRANSFORM_TEX(objectSpacePos.xz, _MainTex);
                fixed4 col = tex2D(_MainTex, objectSpaceUV);
                return col * _Color;                      
            }
            ENDCG
        }
